...
class Slider extends React.Component {
    constructor(props) {
        super(props); //вызывает род констр
        this.state = { //собственное св-во state в виде объекта
            slides: [
                {
    ...
    componentDidMount() { //когда элемент появится на стр
        this.interval = setInterval(() => this.intervalBetweenSlides(), 3000)

    }
    componentWillUnmount() { //когда элемент исчезнет со стр\
        clearInterval(this.interval)

    }
    intervalBetweenSlides() { //автопереключение слайдов
    ...
    toggleAutoPlay() {
        this.setState({
            autoplay: !this.state.autoplay  //прием для переключателей
        })
    }
    nextOne() {
